...
router.post('/buy', auth, async (req, res) => {
  try {
    ...
    // One simple logic block: check cash, subtract balance, add shares.
    user.walletBalance = Number((user.walletBalance - total).toFixed(2));

    let holding = await Holding.findOne({ user: user._id, ticker: stock.ticker });
    if (!holding) {
      holding = new Holding({ user: user._id, ticker: stock.ticker, shares: 0 });
    }
